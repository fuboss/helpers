...
namespace Assets.Code.Tools
{
    public class Math3d : MonoBehaviour
    {
        ...
        /// <returns></returns>
        public static Tuple<Vector3, Vector3> LineCircleIntersection(Vector3 segment1, Vector3 segment2, Vector3 center, float radius)
        {
            var offsetX = center.x;
            var offsetZ = center.z;

            var x1 = segment1.x - offsetX;
            var y1 = segment1.z - offsetZ;
            var x2 = segment2.x - offsetX;
            var y2 = segment2.z - offsetZ;
            ...
            var D = x1 * y2 - x2 * y1;
            var sqrt = Mathf.Sqrt(radius * radius * dr * dr - D * D);

            var resultX1 = (D * dy + Mathf.Sign(dy) * dx * sqrt) / (dr * dr);
            var resultY1 = (-D * dx + Mathf.Abs(dy) * sqrt) / (dr * dr);
            var resultX2 = (D * dy - Mathf.Sign(dy) * dx * sqrt) / (dr * dr);
            var resultY2 = (-D * dx - Mathf.Abs(dy) * sqrt) / (dr * dr);

            var result1 = new Vector3(resultX1 + offsetX, (segment1.y + segment2.y) / 2, resultY1 + offsetZ);
            var result5 = new Vector3(resultX2 + offsetX, (segment1.y + segment2.y) / 2, resultY2 + offsetZ);

            return new Tuple<Vector3, Vector3>(result1, result5);
        }

        public static Vector2? Two2DSegmentsIntersectionPoint(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) 
          {
            ...
            if (xi < Mathf.Min(x1,x2) || xi > Mathf.Max(x1,x2)) return null;
            if (xi < Mathf.Min(x3,x4) || xi > Mathf.Max(x3,x4)) return null;
            if (yi < Mathf.Min(y1, y2) || yi > Mathf.Max(y1, y2)) return null;
            if (yi < Mathf.Min(y3, y4) || yi > Mathf.Max(y3, y4)) return null;
            return p;
           }
        ...
        public static Tuple<Vector3, Vector3> GetClosestEdge(Vector3[] meshVertices, Vector3 point)
        {
            Tuple<Vector3, Vector3> result = new Tuple<Vector3, Vector3>();
            float closestDist = Single.MaxValue;
            for (int i = 0; i < meshVertices.Length; i++)
            {
                var next = i == meshVertices.Length - 1 ? 0 : i+1;
                var p = GetPointLinePerpendicular(meshVertices[i], meshVertices[next], point);
           
                if (p.HasValue)
                {
                    var dist = Vector3.SqrMagnitude(p.Value - point);
                    if (dist < closestDist)
                    {
                        closestDist = dist;
                        result = new Tuple<Vector3, Vector3>(meshVertices[i], meshVertices[next]);
                    }
                }
            }
            return result;
        ...
        // Return point that is perpendicular to given on a segment. If original point lies away from segments returns null
        public static Vector3? GetPointLinePerpendicular(Vector3 linePoint1, Vector3 linePoint2, Vector3 point)
        {
        ...
        {
            var angle = Mathf.Atan2(@from.x * to.z - to.x * @from.z, @from.x * to.x + @from.z * to.z);
            return angle * Mathf.Rad2Deg;
        }

        public static float Angle2DUnsigned(Vector3 from, Vector3 to)
        {
            return Mathf.Abs(Angle2D(from, to));
        }
